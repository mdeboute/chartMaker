...
# my modules
# import script to create DataFrame of from a console session
import dataframe as cd
...
def make_spider(df, col, col_num, title, color):
    """
    plot user's stats into a spide chart (support func to fig_setup)
    ...
    # len(index) sets number of xticks (number of statistics)
    ...
    plt.xticks(angles, labels=index, color='grey', size=8)  # angles identifies the position of the ticks, labels identifies the name of the ticks

    # extracts column from the dataframe and turns it into a list
    df_columns = [float(n) for n in df[col].tolist()]  # list containing current column (column = stat's name list)
